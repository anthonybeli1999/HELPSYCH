...
firebase.auth().onAuthStateChanged(function (user) {
    ...
    if (user && window.location.pathname == "chatusuarios") {
...
function writeUserData(message) {
    ...
    var db_ref0 = firebase.database().ref('Messages').child(usuari1).child(usuari2);
    var key = db_ref0.push().key;

    var db_ref3 = firebase.database().ref('Messages').child(usuari1).child(usuari2);
    var db_ref4 = firebase.database().ref('Messages').child(usuari2).child(usuari1);
    ...
    firebase.database().ref('Messages').child(USERPRUEBA).child('ZRXaGXqLEec8VobrTAhD0bkseP92').on('child_added', function (data) {
        var type;
        if (data.val().from == USERPRUEBA) {
...
function add_data_table(name, pic, email, key) {
    $("#card-list").append(
        ...
        key +
        '" class="card-footer-item btnEdit">Chat</a><a href="#" class="card-footer-item btnRemove"  data-key="' +
        key +
